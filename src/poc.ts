...
export const rchainW = <E2, A, B>(f: (a: A) => Result<E2, B>) => <E1>(ma: Result<E1, A>): Result<E1 | E2, B> => {
  ...
  if (TH.isRight(fb)) {
    return TH.both(w, fb.right)
  }
  // last semigroup
  return TH.both(fb.left, fb.right)
}
...
export const number: numberUD = {
  _tag: 'numberUD',
  decode: (u) => (typeof u === 'number' ? TH.right(u) : TH.left(leafE({ _tag: 'NumberE', actual: u })))
}
...
export interface NaND<D> extends Decoder<InputOf<D>, NaNLE | ErrorOf<D>, number> {
  readonly _tag: 'NaND'
  readonly decoder: D
}

export const nan = <D extends Decoder<any, any, number>>(decoder: D): NaND<D> => ({
  _tag: 'NaND',
  decoder,
  decode: (i) =>
    pipe(
      decoder.decode(i),
      rchainW((n) => (isNaN(n) ? TH.both(nanLE, n) : TH.right(n)))
    )
})
...
export const refine = <From extends AnyD, B extends TypeOf<From>, E>(parser: (a: TypeOf<From>) => Result<E, B>) => (
  ...
  from,
  parser,
  decode: (i) => pipe(from.decode(i), rchainW(parser), TH.mapLeft(refineE))
})
...
export const toTreeWith = <E>(toTree: (e: E) => Tree<string>): ((de: DecodeError<E>) => Tree<string>) => {
  const go = (de: DecodeError<E>): Tree<string> => {
    switch (de._tag) {
      ...
      case 'RefineE':
        return tree(`cannot decode to refinement`, [go(de.error)])
      default:
        // etc...
        return tree('TODO')
    }
...
// export const warningsStruct = number

// const mynumber = pipe(
//   nan(number),
//   refine((n) => TH.both(unexpectedKey('a'), n))
// )

// pipe(mynumber.decode(NaN), draw, print, console.log)
