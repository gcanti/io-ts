...
const undef: _.UnknownDecoder<undefined> = _.fromGuard(undefinedGuard, 'undefined')

const NumberFromString: _.UnknownDecoder<number> = pipe(
  _.string,
  _.parse((s) => {
...
const Positive: _.UnknownDecoder<Positive> = pipe(
  _.number,
...
const Int: _.UnknownDecoder<Int> = pipe(
  _.number,
...
describe('UnknownDecoder', () => {
  ...
  it('compose', () => {
    ...
    const decoder = pipe(_.number, _.compose(D.fromRefinement((n): n is Int => Number.isInteger(n), 'IntFromNumber')))
    assert.deepStrictEqual(decoder.decode(1), _.success(1))
  ...
  const lazyDecoder: _.UnknownDecoder<A> = _.lazy('A', () =>
