...
const undef: _.UnknownTaskDecoder<undefined> = _.fromGuard(undefinedGuard, 'undefined')

const NumberFromString: _.UnknownTaskDecoder<number> = pipe(
  _.string,
  _.parse((s) => {
...
const Positive: _.UnknownTaskDecoder<Positive> = pipe(
  _.number,
...
const Int: _.UnknownTaskDecoder<Int> = pipe(
  _.number,
...
describe('UnknownTaskDecoder', () => {
  ...
  it('Alt', async () => {
    const decoder = _.Alt.alt<string | number>(_.string, () => _.number)
    assert.deepStrictEqual(await decoder.decode('a')(), D.success('a'))
  ...
  it('compose', async () => {
    ...
    const decoder = pipe(_.number, _.compose(TD.fromRefinement((n): n is Int => Number.isInteger(n), 'IntFromNumber')))
    assert.deepStrictEqual(await decoder.decode(1)(), D.success(1))
  ...
  const lazyDecoder: _.UnknownTaskDecoder<A> = _.lazy('A', () =>
