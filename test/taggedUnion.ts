...
describe('taggedUnion', () => {
  ...
  describe('decode', () => {
    ...
    it('should handle intersections', () => {
      ...
      assertFailure(T.decode({ type: 'B' }), ['Invalid value undefined supplied to : T/1: B/b: number'])
    })
    ...
    it('should handle recursive codecs', () => {
      ...
      assertFailure(T.decode({ type: 'B', forest: [{ type: 'B' }] }), [
    ...
    it('should support mixed string, numeric and boolean tags', () => {
      ...
      assertFailure(T.decode({ type: 'a', a: 1 }), [
        'Invalid value 1 supplied to : T/0: { type: "a", a: string }/a: string'
      ])
      assertSuccess(T.decode({ type: 1, b: 1 }))
      assertFailure(T.decode({ type: 1 }), [
        'Invalid value undefined supplied to : T/1: { type: 1, b: number }/b: number'
      ])
      assertSuccess(T.decode({ type: true, c: false }))
      assertFailure(T.decode({ type: true }), [
